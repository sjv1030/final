...
wtc_df = wtc_df.sort_values(by=['month_timestamp'])
wtc_df_stats = pd.DataFrame({"label": ["Minimum", "Maximum", "Mean", "Standard Deviation", "Variance"],
                           "value": [float(wtc_df["oil_val"].min()), float(wtc_df["oil_val"].max()), int(wtc_df["oil_val"].mean()), int(wtc_df["oil_val"].std()), int(wtc_df["oil_val"].var())]})
wtc_df['month_timestamp'] =  pd.to_datetime(wtc_df['month_timestamp'], infer_datetime_format=True)

#multivariate data
ols_df, f_df, fa_df, residual_ols, plot_position_ols, trade_ols = multivariate.getOLS(sym = "o")

# LSTM run
LSTM_prediction, LSTM_future, LSTM_all_data, LSTM_residuals, recommendations = LSTM.LSTM_prediction("oil")
LSTM_prediction_week = LSTM_future.tail(7).round(2)

# get rig_information
wtc_data, ticker = supply_demand_data.get_supply_demand_data('o')
df_stats =  descriptive_statistics.descriptive_stats(wtc_data)
df_stats_df = df_stats.stats_df()
wtc_stats = df_stats_df.reset_index().round(2)

# FB Prophet Data
wtc_daily_df, fb_yu, low, medium, high = fb_prophet.testRunProphet('o')
wtc_daily_df = wtc_daily_df.sort_values(by=['day_timestamp'])
headers = ["ds", 'yhat']
fb_prophet_forecast = medium.reset_index()
fb_prophet_prediction = fb_prophet_forecast[headers].tail(7).round(2)
wtc_daily_df_stats = pd.DataFrame({"label": ["Minimum", "Maximum", "Mean", "Standard Deviation", "Variance"],
                           "value": [float(wtc_daily_df["oil_val"].min()), float(wtc_daily_df["oil_val"].max()), int(wtc_daily_df["oil_val"].mean()), int(wtc_daily_df["oil_val"].std()), int(wtc_daily_df["oil_val"].var())]})
...
def get_menu():
    menu = html.Div([
        dcc.Link('Overview   ', href='/apps/app_oil/overview', className="tab first"),
        dcc.Link('Multivariable Model   ', href='/apps/app_oil/multivariable', className="tab"),
        dcc.Link('FB Prophet   ', href='/apps/app_oil/fb_prophet', className="tab"),
        dcc.Link('Long Short-Term Memory   ', href='/apps/app_oil/LSTM', className="tab"),
        dcc.Link('Takeaways   ', href='/apps/app_oil/takeaways', className="tab")
    ], className="row ")
    return menu
...
overview = html.Div([  # page 1
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    ...
                    html.P("\
                            ...
                            comprehend the change in prices for Oil Markets. We will be \
                ...
                html.Div([
                    html.H6(["Oil Spot Data Statistics"],
                            className="gs-header gs-table-header padded"),
                    html.Table(make_dash_table(wtc_df_stats))
                ], className="six columns"),
            ...
            html.Div([
                html.Div([
                    html.H6('Oil Spot Prices Over the Years',
                            className="gs-header gs-text-header padded"),
                    dcc.Graph(
                        id = "graph-1",
                        figure={
                ...
                html.Div([
                    ...
                    dcc.Graph(
                        ...
                        figure={
                            'data': [
                                go.Scatter(
                                    x=wtc_data.index, # assign x as the dataframe column 'x'
                                    y=wtc_data['twd'],
            ...
            html.Div([
                html.Div([
                    html.H6('Oil Production Statistics',
                            className="gs-header gs-table-header padded"),
                    html.Table(make_dash_table(wtc_stats))
                ], className="six columns"),
                html.Div([
                    html.H6("Oil Information",
                            className="gs-header gs-table-header padded"),
                    dcc.Graph(
                        id='graph-3',
                        figure={
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.H6(["Oil Spot Data Statistics"],
                            className="gs-header gs-table-header padded"),
                    html.Table(make_dash_table(wtc_daily_df_stats))
                ], className="six columns"),
            ...
            html.Div([
                html.Div([
                    html.H6('Oil Spot Predicted Using FB Prophet',
                            className="gs-header gs-text-header padded"),
                    dcc.Graph(
                        id = "graph-1",
                        figure={
                            'data': [
                                go.Scatter(
                                    x=wtc_daily_df.tail(529)['day_timestamp'],
                                    y=wtc_daily_df.tail(529)['oil_val'],
                                    mode = "lines",
                                    name = 'Actual'),
                                go.Scatter(
                                    x = low['ds'][-30:],
                ...
                html.Div([
                    ...
                    dcc.Graph(
                        ...
                        figure={
                            'data': [
                                go.Scatter(
                                    x=wtc_daily_df.tail(500)['day_timestamp'],
                                    y=wtc_daily_df.tail(500)['oil_val'],
                                    name = 'Actual',
                                    mode = "lines"
                                ),
                                go.Scatter(
...
LSTM = html.Div([  # page 4
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.H6('Oil Spot Predicted Using LSTM',
                            className="gs-header gs-text-header padded"),
                    dcc.Graph(
                        id = "graph-1",
                        figure={
                            'data': [
                                go.Scatter(
                                    ...
                                    y = LSTM_all_data['oil_val'],
                                    line = {"color": "rgb(53, 83, 255)"},
                                    mode = "lines",
                                    name = 'Actual'),
                                go.Scatter(
                                    x = LSTM_all_data['month_timestamp'][-7:],
                                    y = LSTM_all_data['oil_val'][-7:],
                                    line = {"color": "rgb(255, 0, 0)"},
                                    mode = "lines",
                                    name = 'Predicted')
                            ],
            ...
            html.Div([
                ...
                html.Div([
                    ...
                    html.P("Note that the Multivariate and FB Prophet models both utilize variables\
                     related to the climate of the US Economy or of the Oil Industry as a whole.\
                     ...
                     to determine future prices."),
                      ...
                      provide insight on the trends for Oil pricing in the months to come.")
                ], className="six columns"),
